...
Current SDK version: **v1.4.1** (defined in `Include/themarketrobo/Core/CSDKConstants.mqh` as `TMKR_SDK_VERSION` — the changelog lives in the comment block above that define). v1.4.1 (2026-09-17) lets an EA ship **no DLL imports at all**: an EA never *calls* `wininet.dll` or `kernel32.dll` (it uses `WebRequest()`), but `Services/CHttpService.mqh` included `CWinINetHttpService.mqh` unconditionally and MetaEditor records an import-table entry for every imported DLL whether or not the code runs — so every SDK-integrated EA shipped as a product the terminal reports as requiring DLL imports, and a customer running with "Allow DLL imports" off saw a warning on a robot that needs none. Defining `TMKR_NO_WININET` before the SDK include now compiles out the include, the `post_wininet()` declaration and its body, so neither `#import` block reaches the binary; both dialects of `Experts/sample-ea/SampleTMRBot` define it. 🚫 Indicators must **not** — `WebRequest()` returns 4014 from indicator context, so WinINet is their only transport, and `CWinINetHttpService.mqh` `#error`s if it is reached with the macro defined (an indicator that somehow reaches `post()` is refused with `TMKR-3011` naming the macro). No behaviour change when the macro is undefined, which is every pre-1.4.1 build; the annotated `v1.4.1` tag is published on `sdk-mql5-lib` and equals the define, and `MIN_REQUIRED_SDK_VERSION` is unchanged (additive). ✅ `SampleTMRBot.ex5` in this repo is the v1.4.1 build (2026-09-20, compiled by MetaEditor on box #3 through `compile-service/scripts/compile_client.py` against the vendored SDK at MQL52026 `d59e9a13`; 184,414 B, sha256 `897d675a…` — the same source with `TMKR_NO_WININET` stripped compiles to 195,920 B). ⚠️ `strings` on an `.ex5` cannot prove the import table is gone (EX5 bodies are encrypted bytecode: the pre-1.4.1 binary that certainly imported both DLLs also shows 0 hits) — the proof is attaching it with "Allow DLL imports" OFF and reading the journal's init line (mql-todos-drain P10, human step 7). Earlier: v1.4.0 (2026-09-12) makes a refused session start say **why**: the SDK reads the RFC 7807 problem `POST /robot/start` returns and alerts a typed reason — `ENUM_TMKR_START_REFUSAL`: license expired, API key not recognized, maximum sessions, rate limited, … — instead of reporting every refusal as `TMKR-3020` "could not connect"; vendor code can read it after `on_init()` with `get_start_refusal()` / `get_start_refusal_code()` (decision D-02). The key refusal stays one reason on purpose: the server answers a mistyped, deleted or rotated key identically. The annotated `v1.4.0` tag is published on `sdk-mql5-lib`, and `MIN_REQUIRED_SDK_VERSION` is unchanged (additive). Earlier: v1.3.3 (2026-09-12) was a **version-identity bump only — no SDK source change** — carried by this repo's first public release (its version and the SDK `#define` move in lockstep, sites 1 and 3 of `tools/gate-sdk-version-consistency.sh`); its SDK tag was never published, and `SDK_RELEASE_PENDING.md` declared that lag until v1.4.0 repaid it. Earlier: v1.3.2 (2026-08-25) restored TLS certificate validation on the indicator transport: `CWinINetHttpService` had built its WinINet flags with `INTERNET_FLAG_IGNORE_CERT_CN_INVALID | _DATE_INVALID` set **unconditionally**, so every request it made — each carrying the vendor API key and the session token — accepted any CA-issued certificate for any hostname, expired or not. Both flags now compile in only under an opt-in `TMKR_INSECURE_TLS_DEBUG` macro that is defined nowhere in the shipped tree (CI job `sdk-tls-flags` fails if they go unconditional again). The EA transport (`CHttpService`/`WebRequest`) was never affected. Earlier: v1.3.1 (2026-07-19) fixed the token-refresh self-DoS: the effective refresh threshold is now clamped to half the token's real lifetime and failed refreshes retry on a 30 s cooldown — previously a threshold ≥ the token lifetime (emitted robots set 300 s against 300 s tokens) produced ~1 `POST /robot/refresh` per second. Earlier: v1.3.0 added the synchronous `ITMKR_SymbolGate` seam; v1.2.1 (2026-07-09) restored the pre-TMKR-rename backwards-compat aliases in `Include/themarketrobo/TMKR_Compat.mqh` — the 2026 rename had shipped without them, which made every legacy-name consumer (these samples included) uncompilable. The Vendor Portal's [SDK Integrator Lambda](#sdk-integrator-pipeline) refuses to ship integrated output for vendors whose local SDK is below `MIN_REQUIRED_SDK_VERSION`.
...
**Sites 4–5 of SDK version identity live outside this repo.** The gate checks four of the six
sites from here; the other two are in the private `aws/` repo, which this repo's CI does not check out:

| Site | Where | What it asserts |
|---|---|---|
| 4 | `aws/src/endpoints/common/sdk-integrator/test/sdk-error-codes.test.ts` | the SDK's generated error catalog matches `catalog.json`, the typed start-refusal mapping follows the codes `/robot/start` emits, and the `TMKR_SDK_VERSION` pin. It self-skips in aws CI and runs only on a laptop with this repo beside `aws/` — so it reads that laptop's **checkout**, not a gitlink |
| 5 | `aws/src/endpoints/common/sdk-integrator/src/lib/sdk-symbols.ts` `MIN_REQUIRED_SDK_VERSION` | the floor below which the integrator refuses vendor output (`1.1.1`; v1.4.0 did not move it) |

`SDK_RELEASE_PENDING.md` exists only while the newest SDK tag lags the define (the site-6 waiver);
delete it in the same change that makes tag == define. There is no waiver for sites 1–3.
