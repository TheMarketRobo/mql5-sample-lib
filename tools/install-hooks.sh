...
# (ci-cd-hardening P13, ledger L-13. Logic is hub's scripts/install-hooks.sh; only the
# script's own path differs, because this repo cannot have a lowercase `scripts/` —
# see the note in tools/verify-local.sh.)
#
...
# `core.worktree = ../../../mql5-sample-lib` — that line is the only thing connecting
# `hub/.git/modules/mql5-sample-lib` to the checkout. Enabling `extensions.worktreeConfig`
...
# the UTF-16LE MQL sources here would be corrupted by the fleet's text hooks. The reason
# is written out in .githooks/commit-msg.
